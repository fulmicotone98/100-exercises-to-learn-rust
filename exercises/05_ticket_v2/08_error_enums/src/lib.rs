...
enum TicketNewError {
    TitleEmpty(String),
    TitleGreater50B(String),
    DescriptionEmpty(String),
    DescriptionGrater500B(String),
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(t) => t,
        Err(TicketNewError::DescriptionEmpty(_))
        | Err(TicketNewError::DescriptionGrater500B(_)) => {
            Ticket::new(title, "Description not provided".to_string(), status).unwrap()
        }
        Err(TicketNewError::TitleEmpty(e)) | Err(TicketNewError::TitleGreater50B(e)) => {
            panic!("{e}")
        }
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleGreater50B(
        ...
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionGrater500B(
